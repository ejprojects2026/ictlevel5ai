    ...
    req.on("end", () => {
      ...
      const respond = () => {
        ...
        if (isGrader) {
          ...
          try {
            const q=(/QUESTION:\s*(.*)/.exec(j.message)||[,''])[1].slice(0,45);
            const st=(/STUDENT:\s*([\s\S]*)$/.exec(j.message)||[,''])[1].slice(0,40);
            console.log('  GRADE#'+(++__n)+' v='+JSON.parse(reply).verdict+' | Q='+q+' | A='+st);
          } catch(e){}
...
try {
  ...
  const probe = await evalJs(`(() => ({
    stubInjected: Array.isArray(window.__speechLog),
    hasSynth: typeof window.speechSynthesis,
    hasUtter: typeof window.SpeechSynthesisUtterance,
    voices: (window.speechSynthesis && window.speechSynthesis.getVoices() || []).map(v=>v.name),
    muteBtnClass: document.getElementById("muteBtn") ? document.getElementById("muteBtn").className : null,
    logLen: (window.__speechLog||[]).length
  }))()`);
  ...
  if (AI_MODE === "hang") {
    ...
    // let the count-up ring animation settle before reading the score
    let prevScore=null, stable=0;
    for (let i=0;i<25;i++){ const cur=(await evalJs(snap)).score; if(cur===prevScore){ if(++stable>=3) break; } else stable=0; prevScore=cur; await new Promise(r=>setTimeout(r,150)); }
    s = await evalJs(snap);
    if (s.screen === "results") ok("11. full class reaches results without blocking", `score=${s.score} (settled)`);
